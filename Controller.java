...
public class Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("home")) {
            ...
            String heartString = request.getParameter("heartrate");
            String date = request.getParameter("date");
            String time = request.getParameter("time");
            String notes = request.getParameter("notes");

            // validate and convert salary string into a double
            if (heartString == null || heartString.isEmpty()) {
                heartRate = 0;
            } else {
                heartRate = Integer.parseInt(heartString);
            }

            // store data in an User object
            User user = new User(0, email, heartRate, date, time, notes);
